...
def match_molecule(s1,s2,napm,mn,cm1=None,create_duplicate=True):
    ...
    while lll<2:
        ...
        if rl>match_molecule_cross_tolerance:
            ...
            resi=residual_single_molecule(s1,s2,napm,0,mn)
            if resi<match_molecule_tolerance:
                print "a rotation is invovled. Final resi=", resi
                return s2
        print "A mirror reflection is involved"
        structure_handling.cell_reflection_z(s2,False)
        trans=[0,0,2*cm1[2]]
        structure_handling.cell_translation(s2,trans,False)
#        print "this is s2 after the mirror reflection", s2.geometry        
        resi=residual_single_molecule(s1,s2,napm,0,mn)
        if resi<match_molecule_tolerance:
...
def is_duplicate (s1,s2,nmpc,napm,create_duplicate=True,extended_check=False):
    ...
    if extended_check:
        s1=copy.deepcopy(s1)
        s2=copy.deepcopy(s2)
        structure_handling.cell_extension(s1,nmpc,napm,create_duplicate=False)
        structure_handling.cell_extension(s2,nmpc,napm,create_duplicate=False)
        ex_mul=8
...
def main2():
    ...
    f=open("duplicate_not_quite.data","r")
    st=f.read()
    struct.loads(st)
    f.close()
    structure_handling.cell_extension(struct,nmpc,napm,create_duplicate=False)
    structure_handling.print_aims(struct,"duplicate_not_quite_extended.data")
def main1():
    nmpc=2; napm=10
    struct=structure.Structure()
    f=open("duplicate_0.data","r")
    st=f.read()
    struct.loads(st)
    f.close()
...
def main1():
    ...
    st=f.read()
    struct.loads(st)
    print "this is struct.geometry", struct.geometry
    struct_1=structure_handling.cell_rotation(struct,vec=[1,4,1],deg=20)    
           
#    print "this is properties before", struct.properties
#    struct_1=structure_handling.cell_translation(struct_1,[11,10,10])
    structure_handling.cell_reflection_z(struct_1,False)
    struct_1=structure_handling.mole_translation(struct_1,1,napm,frac=[1,1,1],create_duplicate=True) 
    print "this is before everything struct_1", struct_1.geometry
#    struct=cell_rotation(struct,vec=[0,0,3],deg=180)
    print is_duplicate(struct,struct_1,nmpc,napm,create_duplicate=False)
#    structure_handling.print_aims(struct_1,"beta_glycine_0_1.in")

#    print_aims(struct,"beta_glycine_0_1.in")
#    print "this is properties", struct.properties
