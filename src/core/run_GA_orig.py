...
class RunGA():
    ...
    def start(self):
        ...
        while True:
            ...
            ########## Check if finished ##########
            try: 
                if len(self.structure_coll.structures) - 1 >= self.number_of_structures + len(self.structure_coll.structures):
            ...
            ########## Fill initial pool ##########
            if self.verbose: self.output('Filling initial pool')
            print "Filling IP"
	...
	#    break            
	 ...
	 ########## Perform Genetic Algorithm Tasks ##########
	    ...
	    print "Beginning structure selection"	
	    ...
	    ########## Mutation Decision ##########
   	    ...
   	    print "HEY", new_struct.get_property('alpha')	
...
#             ########## Mutation Execution ##########
#
#		#FARREN TODO: Come back to later- 
...
#	    child_number = 0
		...
		temp_min_e = min(temp_e_list)
            ...
            else: self.end_of_iteration_tasks(structures_to_add, temp_min_e)	
            end_time = datetime.datetime.now()
            if self.verbose: self.output("Iteration time: -- " + str(end_time - begin_time))

    def end_of_iteration_tasks(self, structures_to_add, min_e):
