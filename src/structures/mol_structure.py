...
class Structure(object):
	...
	self.lats = numpy.zeros(0, dtype=[('latx', 'float32'), ('laty', 'float32'), ('latz', 'float32')])
    ...
    def build_lats_by_atom(self, latx, laty, latz):
        # increase the size of the array
        size = self.lats.size
        self.lats.resize(size + 1)
        # assign values
        self.lats[size]['latx'] = latx
        self.lats[size]['laty'] = laty
        self.lats[size]['latz'] = latz
    ...
    def build_lats_by_whole_atom(self, lat):
        # increase the size of the array
        size = self.lats.size
        self.lats.resize(size + 1)
        # assign values
    ...
    def build_lats_whole(self, lats): self.lats = lats
	...
	def add_previous_lat(lat):
	    self.build_lats_by_atom(lat['latx'], lat['laty'], lat['latz'])
	...
	while True:
	    try: line = lines_iter.next().split()  # read each line
            except: add_previous_lat(lat); return self.lats
	    if len(line) == 0: continue
            if '#' in line[0]: continue  # skip commented lines
            if line[0] == 'lattice_vector':
		if not len(atom) == 0: add_previous_lat(lat)
	        lat = {}
 		lat['latx'] = float(line[1])
		lat['laty'] = float(line[2])
		lat['latz'] = float(line[3])
...
#	lat = {}
       # while True:
        #    try: line = lines_iter.next().split()  # read each line
       #     except: add_previous_atom(atom); add_previous_lat(lat); return self.geometry ; return self.lats
#	   #except: add_previous_lat(lat); return self.lats
 ...
 #               atom['fixed'] = True
    ...
    def get_lats(self): return self.lats   
	...
	data_dictionary['lats'] = self.lats.tolist()
	...
	self.build_lats_whole(convert_array(data_dictionary['lats']))
