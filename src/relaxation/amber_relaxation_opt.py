...
# 		data_string +=str(j+1)  + ' ' + str(geo[j]['x']) + ' ' + str(geo[j]['y']) + ' ' + str(geo[j]['z']) 
	...
	ay=0
	az=0
	bx=np.cos(gamma*rad)*b
	by=np.sin(gamma*rad)*a
	bz=0
	cx=c*np.cos(beta*rad)
	cy=(b*c*np.cos(alpha*rad)-bx*cx)/by
	cz=np.sqrt(np.absolute(c**2-cx**2-cy**2))
	lata_out = np.zeros(3)
	latb_out = np.zeros(3)
	latc_out = np.zeros(3)
	lata_out[0] = ax
...
# 	out_geom.close() 	
	...
	while True:
		...
		if 'Function' in line:
			...
			print line
			print line[4]
			self.result_struct.set_property('energy',float(line[4]))
			return float(line[4])  # final energy
