...
class FHIAimsRelaxation():
    ...
    def execute(self):
        ...
        '''
        # creates shell script with proper syntax by copying jobs-file from the main folder, defines run settings in it, and runs it as a subprocess
	exe_jobs = open(self.working_dir + "/../../jobs", 'r').readlines()
	exe_string = ''
	for line in exe_jobs:
		if ("run_GA.py" in line) is True: continue
		else: exe_string += line
	exe_string += 'binary="' + self.ui.get('FHI-aims', 'path_to_aims_executable') + '"\n'
	exe_string += 'mpiexec -n 6 $binary > aims.out' 
##'mpirun -np ' + self.ui.get('FHI-aims', 'number_of_processors') + ' ' + self.ui.get('FHI-aims', 'path_to_aims_executable') + ' > ' + 'aims.out'
